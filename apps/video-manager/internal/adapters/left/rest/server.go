...
func (s *server) ListenAndServe(youtubeDownloader application.YoutubeDownloaderPorts) error {
	...
	s.l.Message("Server ready to receive REST requests, on port: " + strconv.Itoa(s.cfg.SharedConfig.VideoManager.Port))
	err := http.ListenAndServe(":"+strconv.Itoa(s.cfg.SharedConfig.VideoManager.Port), handler)
	if err != nil {
		return err
	}
